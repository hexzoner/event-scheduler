            ...
            return (
              <div className="flex row items-start" key={event.id}>
                <ul className="text-center p-4 border min-w-48 h-full">
                  ...
                  <li className="text-xl font-bold text-sky-500">{monthName}</li>

                  <li className="text-lg font-semibold text-slate-500 border-t pt-2 mt-2">{event.date.slice(11, 16)}</li>
                </ul>
                <div className="bg-slate-100 p-4  w-full h-full relative">
                  <p className="text-xl font-bold line-clamp-2">{event.title}</p>
                  <p className="text-md text-slate-500 line-clamp-2">{event.description}</p>
                  <p className="flex align-middle text-md font-semibold my-4">
                    <span>
                      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" className="bi bi-geo-alt mr-2 mt-1" viewBox="0 0 16 16">
                  ...
                  <div className="absolute bottom-0 right-0 px-4 py-1 bg-sky-950 text-white text-sm">{eventYear}</div>
                </div>
              </div>
            );
          })}
